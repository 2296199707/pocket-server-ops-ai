...
- Agent 正式运行不设置固定的模型步骤数或工具调用总量上限；模型正常结束、用户取消或真实请求/工具错误才结束当前轮次。`maxSteps` 和 `maxToolCalls` 仅在测试或部署明确传入时生效，不能把长任务强行标记为 `unknown`；单次模型响应的工具数组解析保护仍属于协议解析边界，不是任务级中断；
...
- Agent 不再用隐含的步骤数或工具调用数作为任务控制边界；协议错误、实际请求/工具错误、取消和权限审批仍按各自语义处理。Agent 仍可按工具定义执行服务器命令和文件操作，确认模式由任务设置决定。
...
- 已移除正式 Agent 的隐含步骤数和工具调用总量上限，长任务不再因达到 `64/128` 被强行
  标记失败或未知。
- 已清理会直接截断合法长任务的人工截止：Responses/Chat 请求空闲超时、恢复模式
  `terminal.exec` 两分钟命令超时、底层 `SshConnection.run` 两分钟超时、SFTP/MCP/图片请求
  两分钟超时，以及子代理 `wait_agent` 的默认/最大等待截止。详细位置、处理状态和保留边界见
  `docs/codex-source-audit-mobile-agent-v1.zh-CN.md` 的“长任务稳定性审查进度”。
- 协议完整性、用户取消、审批拒绝、远程进程数量、输出/文件分块等边界不属于同类过度限制，
  保留并单独记录，避免为了长任务稳定而造成无限内存或重复执行远程副作用。
...
### 2026-08-30：手机端长任务收尾与 WFL 能力边界

本轮将 WFL/Codex 的生命周期结论落到手机端最小实现，记录如下，后续从本节继续审查即可。

- 对话追加任务使用 `Task.pendingInputs` 持久化下一轮输入，支持文本和附件；当前 turn 完成
  后顺序消费，成功才写 `task.input_consumed`，失败、取消和重启保留未消费输入。输入框运行中
  保持可编辑。它是手机端稳定队列，不宣称等同于 Codex app-server 的 `turn_steer`。
- 子代理正常执行仍不加任务时长上限；只有 cancel/close/资源清理使用约 5 秒生命周期等待。
  超时节点进入 `unknown` 并唤醒等待者，晚到结果不覆盖未知状态，也不能在旧运行未释放时启动
  follow-up，避免死等和同一子代理重复执行。
- SFTP 控制操作单次 15 秒、分块读写单块 30 秒；整文件读写和复制无总时长上限。删除、工具
  释放、dispose 使用有限等待并继续其他资源清理，不把清理失败伪装成任务成功。
- `terminal.start` 在连接建立前预留并发槽位，修复第 65 个并发启动因异步登记延迟而等待超时
  的问题。该 64 是手机资源上限，不是 Agent 任务步骤或工具调用上限。
- `AgentLoop` 仍只用无进展检测、用户取消和真实模型/工具错误结束普通任务；远程命令不会因
  清理超时自动重放。已完成 122 项聚焦测试，`git diff --check` 通过，静态分析仅保留已有
  `test/mcp_client_test.dart:176:39` 风格提示；本轮未构建 APK、未提交或发布。
