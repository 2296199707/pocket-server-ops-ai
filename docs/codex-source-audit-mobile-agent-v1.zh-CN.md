...
- Agent 每一轮向当前对话绑定的全部服务器提供一套稳定的远程工具；多服务器调用必须带
  `server_id`，当前活动服务器只作为默认工作目录和默认目标，不再是能力边界。终端、服务器
  文件管理器、仪表盘和聊天状态入口仍按当前功能选择服务器。切换 Agent 服务器会保留对话
  历史并记录配置变更。
- 多服务器工具由 `RemoteAgentToolsGroup` 路由到各自的 SSH runtime；`terminal.start` 返回
  的进程 ID 编码了所属服务器，后续 poll/write/stop 会自动回到同一 runtime。新增
  `server.transfer`，通过手机在源服务器和目标服务器之间分块流式传输单个二进制文件，
  不把文件内容放入 AI 上下文；普通文件工具仍按 `server_id` 访问目标服务器。
- 每台服务器的 SSH 连接池 key 为“任务 ID + 服务器 ID”，远程写操作按服务器分别排队；不同
  服务器不会因为共用一个对话而误用连接或共享工作目录。
...
当前结论：多服务器绑定、Agent 工具路由、服务器间文件传输、功能级选择持久化和按服务器
缓存已实现。后续只有服务器绑定/路由、连接池、传输协议、目录/仪表盘缓存、页面异步生命
周期或对应测试改变时才重新审查本条，不因普通模型或 UI 修改重复查询。
...
### 2026-08-29：多服务器 Agent 路由与手动刷新推理参数

- `lib/agent/agent_tools.dart:RemoteAgentToolsGroup` 是多服务器的唯一工具入口。它复用单
  服务器工具实现，只在工具 schema 中增加 `server_id`；单服务器仍允许省略该字段，多个
  绑定服务器时由 schema 和运行时同时要求明确目标。`terminal.start` 的返回 ID 使用
  `multi:<base64-server-id>:<process-id>`，因此进程后续轮询、输入和停止不会串到别的服务器。
- `server.transfer` 只在至少两台服务器绑定时暴露，源和目标服务器 ID 都必须明确；源文件
  通过 `readFileBytesChunk` 读取，目标通过现有 `ResumableFileUploader` 写入和提交。它不
  生成文件文本结果，也不把二进制放入模型消息。
- `AppController` 为每个“任务 + 服务器”使用独立 SSH 连接池 key，并按服务器分别串行化
  远程写入；任务配置中的服务器列表变化会释放整个旧 runtime 组。事件优先从工具参数或
  结果保存实际 `server_id`/`server_ids`，而不是错误地统一标成活动服务器。
- 模型抽屉的“刷新模型列表”和“刷新推理参数”是两个手动动作。打开抽屉、模型页和子代理
  页不会自动请求供应商；切换 OpenCode Go 等模型后，用户点击对应刷新按钮才重新读取当前
  供应商模型目录并更新推理能力缓存。子代理使用独立供应商时刷新其供应商，跟随父代理时
  更新当前对话供应商的缓存；请求失败继续显示原缓存并提示失败。
- 这次没有改变 Responses/Chat Completions 选择、供应商配置或上下文压缩协议；推理刷新
  仍调用已有 `loadProviderModelMetadata`，不引入第二套供应商接口。

定向测试：`test/remote_process_test.dart` 覆盖多服务器命令路由、带服务器标识的长任务
进程和跨服务器二进制分块传输；`test/ui/chat_page_test.dart` 覆盖抽屉打开不请求、模型
列表刷新和推理参数手动刷新失败提示。实现完成后再以 `flutter analyze` 和这两组测试作为
本条基线；未在本轮发布 beta 或 Release。

当前结论：多服务器 Agent 和手动推理参数刷新已实现。后续只有远程工具 schema、服务器
连接池/传输、模型目录缓存、刷新按钮行为或对应测试改变时才重新审查本条，不因普通 UI
或模型名称变化重复查询。

### 2026-08-29：远程工具连接错误交回 Agent

- 远程工具调用失败时，`AgentLoop` 不再因为 `callOperationStarted` 直接产生
  `task.unknown` 并结束当前 Agent 轮次。错误仍记录为 `tool.failed`，同时作为工具结果返回
  给模型，并明确提醒“不要直接重复执行，先确认服务器当前状态，再决定是否重试”。
- `remoteOperationStarted` 仍保留用于 AI 请求自身失败、步骤上限等无法继续判断的情况；本次只
  放宽远程工具异常路径，不改变用户取消、权限审批和 AI 请求失败的处理。
- 回归测试：`test/agent_loop_test.dart` 验证远程写工具抛出连接错误后模型仍会收到工具结果并继续
  下一次请求；`test/remote_process_test.dart` 验证首次 SSH 失败后后续工具调用可以重新建连。
- Agent 的初始 SSH 建连现在也是延迟的：启动 Agent 时先创建带有远程工具定义的循环，真正调用
  远程工具时才通过连接池建连。首次连接失败会走同一条 `tool.failed` 结果路径，模型可以先确认
...
- Agent Loop 已为每次 AI 请求和工具完成事件记录 `elapsed_ms`，用于定位模型、网络或
  工具执行耗时。
- 同一轮中明确标记为只读、无需审批的 `terminal.poll` 与 `file.read` 会提前并发启动，
  但仍按模型返回的调用顺序回填工具结果。
- 写操作、命令执行、安装、删除、移动和需审批工具继续串行，避免副作用重复或依赖
  顺序被破坏。
- 该版本曾将 `agent.timing` 标成非阻塞诊断事件，并新增 Agent Loop 测试验证两个独立
  只读工具确实重叠执行。2026-09-09 复核发现 timing 仍进入 controller 普通事件队列，
  因而并未真正移出关键路径；本次已将计时合并进 `assistant.completed`，详见下方实现记录。
- 事件持久化路径已统一使用 `saveTaskAndEvent` 事务，任务快照与事件一次写入，减少
  普通事件路径的独立 SQLite 写入次数，同时保持任务状态和事件原子一致。
- 同一任务的事件序号增加内存递增缓存；首次写入从数据库读取最大序号，后续事件不再
  每次查询 `MAX(sequence)`，事件队列仍保证串行分配，重启后会从数据库重新接续。
- 连续事件的 Flutter 通知改为 16ms 合并一次；数据库写入和内存状态更新仍立即完成，
  只减少重复的界面重建，不改变 Agent 上下文或事件顺序。
...
- Agent Loop 已记录每次 AI 请求和工具完成耗时，用于对照并发改造前后的真实数据；
  旧的独立 `agent.timing` 事件在 2026-09-09 改为 assistant 完成事件中的 `timing` 字段。

### 2026-09-09：Agent 对话速度增量审查与首轮实现

- 审查基线：`beta` 的 `7eb07f2`，版本 `1.0.5-beta.13+57`。
- 本次只检查 APP 当前实现，不重新开展 Codex 全量源码研究；下列是 APP 的代码事实
  和待选方案，不把代码注释当作 Codex 官方行为的证明。
- 进度：请求、工具、SSH、上下文组装和 UI/存储已检查；独立复核已汇总并核对源码。
  本轮完成了流式路径、SSH 轮询和 Agent 工具调度的首轮实现；没有调用真实供应商、构建或发布。
- 未做手机端性能采样或供应商端到端对照，不能把下列结构性开销换算成确定秒数或倍数。
  首字可见时间、整项任务耗时和界面卡顿需要分别测量。

#### 已确认的剩余开销

1. **P1：Responses 实际上缓存整次响应后才显示流式文字。**
   `lib/agent/openai_compatible_client.dart:657` 的 `_sendOnce` 将 delta 放进
   `attemptDeltas`，直到 `_readSse` 成功结束后才在第 718 行一次补发。
   `lib/agent/agent_loop.dart:243` 又为这些 delta 建立 Future 链，并在第 250 行
   等待链结束。因此“供应商已开始输出”不等于“手机已经显示首字”；长回复结束时还会
   集中处理增量。这首先是可见响应延迟，不代表模型自身生成更慢。
   建议将当前 attempt 的文本作为临时草稿即时显示，成功后才写入完整历史；重试时
   撤销失败 attempt 的草稿，不删除已确认上下文，不执行未完成的工具参数。
   `lib/app_controller.dart:2676` 的 `handleAiRetry` 已清理临时文本，可复用，但要
   核对排队 delta、取消和重试的顺序，不能只删掉缓存就宣布完成。

2. **P1：SSH 结果轮询包含可避免的串行读和重复 SFTP 通道建立。**
   `lib/ssh/remote_process.dart:185` 已取得探测脚本的状态，仍在第 190、191 行
   依次读取 stdout 和 stderr；脚本同时返回的 `stdout_size`、`stderr_size`
   没有用于跳过无新增内容的读取。每次读取通过
   `lib/ssh/ssh_connection.dart:1232` 的 `_withSftp` 新开并关闭一个 SFTP 通道，
   还会打开文件、stat、读取、关闭文件。SSH 连接本身已有池，不应误写为每次重连 SSH。
   建议先按探测大小跳过本轮无增量的流，再将独立输出读取合并到同一读取批次；
   优先批次内复用通道，不先建立复杂的全局通道池。探测后新增的字节留给下一轮，
   保持偏移、退出码和断线恢复，不丢输出、不自动重放命令。

3. **P1：首轮模型请求前串行等待服务器准备和项目指令扫描。**
   `lib/app_controller.dart:2980` 的多服务器循环逐台 await 连接；第 3149 行再
   逐台 await 指令加载。单服务器也会在第 3174 行等待指令加载，然后才进入模型请求。
   已有 SSH 池可以复用热连接，但第 3411 行起的 finally 在没有托管进程运行时
   调用 `_releasePhoneTask`（第 7715 行），释放该任务的 SSH 池连接。因此普通任务
   完成后的下一轮通常仍要重新连接，不能将“有池”等同于“跨用户轮次一直复用”。
   AI HTTP 客户端也在每个用户轮次结束时关闭，同一轮内部的多次模型请求则会复用。
   `lib/agent/remote_instructions.dart:30` 每次向父目录查找 `.git` 都列出整个目录，
   再读取 `AGENTS.override.md` / `AGENTS.md`；没有跨轮指令缓存。深目录、大目录或
   多台服务器会累积往返，即使本轮只需要其中一台，第一轮模型也要等准备路径走完。
   建议先把独立服务器的准备并发化，保留首次主机密钥/交互认证的人工确认顺序；
   将目录枚举改为批量检查所需文件，按服务器、工作目录和文件变更复用指令内容。
   缓存必须能发现指令文件新增、删除和覆盖文件优先级变化，不能永久使用旧指令。
   首次必要指令仍应在执行前加载，不直接删掉这个流程。
   连接生命周期可作为后续小项：区分“清理本轮进程”和“断开活跃会话连接”，在明确
   断开、凭证/目标变化或会话淘汰时关闭；不能无限保留所有旧对话的连接，也不能在
   不同身份之间误用客户端。先测连续两轮的握手占比，再决定是否修改生命周期。

4. **P2：长命令使用提示仍偏向额外的 start/poll 模型往返。**
   `lib/app_controller.dart:6647` 提示短命令用 `terminal.exec`，长命令用
   `terminal.start`。后者在 `lib/agent/agent_tools.dart:493` 只返回进程 ID，
   通常要再经过一轮模型请求才调用 poll。
   实际上第 475 行起的 `terminal.exec` 已经“启动 + 等首批结果”，未结束时同样
   返回可继续轮询的进程 ID。建议优先利用 exec 的 yield 能力，start 保留给确实
   需要立即返回或 PTY 的任务；不是删工具，也不是自动降低所有等待时间。
   默认 10000 ms 是最多等待首批输出/完成的时间，探测脚本有输出或结束会提前返回，
   不能声称每条命令固定多等 10 秒。减少的模型轮数要用实际任务验证。

5. **P2：只读并发必须以顺序边界为单位。**
   首轮实现前，`lib/agent/agent_loop.dart` 要求整个工具批次都符合只读并发条件；
   混入一个写操作就整个批次串行。现在调度器只预启动从当前调用开始的连续、明确标记
   为只读且无需审批的段；写操作、审批、未知工具、参数解析失败和不可并发工具都会
   停止预启动扫描。写操作完成后，后面的只读段才会启动，因此不会提前读取旧状态。
   结果仍按模型返回顺序消费并加入上下文。不通过命令字符串猜测任意 shell 命令是否只读，
   不让写后读取提前执行。

6. **P2：含附件的长上下文仍有重复组装成本，收益待采样。**
   `lib/app_controller.dart:7086` 每次新用户轮次重建模型历史后逐个恢复有效附件，
   `_resolveAttachment`（第 7403 行）读文件并重新 base64 编码。
   `lib/agent/openai_compatible_client.dart:506` 每个模型步骤重新构造 input 并
   `jsonEncode` 请求体，大段文字和 base64 会在当前 isolate 上处理。
   自动压缩边界之前的附件已被排除，且同一 Agent 轮内并非每个工具步骤都重新读附件，
   不应夸大为“每次都读所有历史图片”。建议先测组装时间，再决定是否做限定生命周期的
   编码复用或将大请求编码移出 UI isolate；不能以无限内存缓存替代磁盘，不能为了
   提速丢弃图片、清空历史或擅自改动上下文压缩阈值。

7. **P2：长输出和已加载历史会放大 UI/历史投影开销。**
   `lib/app_controller.dart:644` 已用 StringBuffer 和 32 ms 合并流式通知；不是
   每个字符重建整页。但 flush 时仍将整段文本转成 String，
   `lib/ui/chat_page.dart:5111` 先用完整 Text 显示草稿，终态切换为完整 MarkdownBody。
   先恢复真实流式并测量大段 Markdown 的终态帧耗时，再决定是否分块/惰性布局；
   分块只影响绘制，不删除正文、不减少模型上下文，保留整段复制和查看全文。
   `lib/app_controller.dart:1946` 每条持久事件复制任务完整内存事件列表；
   `lib/ui/home_shell.dart:188` 监听全局 controller，
   `lib/ui/chat_page.dart:172` 的列表 identity 缓存失效后，第 4287 行重新配对事件。
   默认只加载最近 40 条，加载许多历史页后才更容易放大此项。建议先把工具配对改为
   单次索引扫描、稳定未变气泡的绘制，后续按采样结果再考虑任务级增量投影，不先重写
   全部状态管理。
   此外，`lib/storage/app_database.dart:692` 的 `loadModelEvents` 每个用户轮次
   查找压缩点并读取有效事件后缀；`lib/app_controller.dart:3343` 的 usage 更新在
   缓存失效时也会扫描历史。候选方案是记录已处理 sequence / 压缩索引并按新增事件
   更新；必须保留供应商切换、压缩后重建和重启恢复语义。实际耗时尚未测量。

8. **P1：上轮 timing 非阻塞优化仍有队列阻塞（本轮已解决）。**
   `lib/agent/agent_loop.dart:253` 虽然 unawaited 发出 `agent.timing`，
   `lib/app_controller.dart:3291` 却只为 `assistant.delta` 旁路；timing 在
   第 3308 行进入普通 `eventQueue`，第 3360 行调用 `appendTurnEvent`，最终走
   第 1945 行的 `saveTaskAndEvent`。随后 Loop await 的 `assistant.completed`
   排在这个 timing 写入之后，所以没有真正消除关键路径上的一次事务。
   建议将诊断留在内存或并入已有完成事件，避免为了诊断额外排一次普通任务事务。
   现有 `elapsed_ms` 还包括请求体组装和 delta 队列，不是纯模型推理时间；并发读
   先启动、后到顺序消费点才记录 toolStartedAt，会低估排后工具的真实执行时间。
   测量改为从实际启动计时，并区分准备、首个文本增量、整次响应和工具执行；不扩展为
   大型遥测系统，不记录密钥或完整请求正文。

#### 建议实施顺序和验证范围

- 第一批：恢复实时草稿；修正 timing 的真实阻塞；精简命令结果轮询；利用已有
  exec/yield 减少无内容往返。
- 第二批：服务器准备与指令加载优化；扩展已确认只读工具的并发。
- 大附件编码、连接跨轮复用和长历史投影优化先采样后决定，避免为小收益扩大修改范围。
- 后续验证只覆盖关键行为：终态前能显示文字、失败重试不重复、不执行半截工具调用；
  空 stderr 不必重复远程读取、有增量不漏读、重连后偏移继续；写后读顺序不改变。
  再用同一模型/推理配置对照首字时间、模型轮数、SSH 调用数和任务总时长。
- 保持用户选定模型、推理强度、协议、审批和重连策略；不通过削减能力伪装性能提升。

#### 2026-09-09：本轮实现结果

- 已完成 Responses 和 Chat Completions 的即时流式草稿：SSE 增量到达即通知 UI；
  只有完整终态进入模型历史。断线重试或取消时，旧 attempt 的晚到增量会被忽略，
  不执行半截工具调用，也不把失败草稿重复写入历史。
- 已移除独立 `agent.timing` 的持久化事件，将请求总耗时和首个文本增量耗时并入
  `assistant.completed.timing`，避免单独增加一次任务事件事务。
- 已完成 SSH 进程轮询优化：使用状态探测得到的 stdout/stderr 大小跳过无新增读取；
  两路均有新增时通过一次 SFTP 会话读取，保持字节偏移、分块、断线恢复和退出状态。
- 已把已确认只读的项目、本地、Windows 电脑文件读取/目录列表，以及多服务器包装层
  标记为可并发候选；AgentLoop 现在按连续只读段并发，写操作仍串行，混合批次仍保持
  顺序。新增边界测试覆盖 `read → write → read`：前一只读段重叠，写后读取不会提前启动。
- 已在 Responses 和 Chat Completions 请求中明确发送 `parallel_tool_calls: true`，与
  当前固定 Codex 源码的请求构造一致；这只允许模型在一轮返回独立工具调用，实际
  是否并发仍由 APP 的工具声明和读写顺序边界决定，不会把写操作并行化。
- 多服务器首次指令读取改为并发准备并按服务器原顺序合并；Agent 提示优先使用
  `terminal.exec` 的 yield 能力拿到首批输出，`terminal.start` 仍保留给 PTY 或需立即返回的场景。
- 验证：128 项定向测试通过（流式、Agent Loop、控制器、两种 AI 协议、SSH 进程、连接、
  指令、多服务器和电脑工具）；静态分析只剩既有的 `test/mcp_client_test.dart:176`
  `use_null_aware_elements` 信息，不属于本轮改动。未构建 APK，未调用真实供应商。
- 在追加并行工具请求字段及更严格的 `read → read → write → read → read` 顺序断言后，
  协议与 AgentLoop 定向测试共 76 项通过；它们与前述 128 项跨模块回归互补。
- 后续仍待真实设备采样：跨用户轮次 SSH 连接生命周期、长历史投影、附件 base64 编码、
  大段 Markdown 终态布局。它们暂不改，避免为了未经测量的小收益扩大范围。

### 2026-09-09：请求历史传输策略决策

- 决策：不采用 `previous_response_id`、供应商服务端会话链、WebSocket 状态续接或其他
  依赖供应商保存对话状态的请求历史优化。
- 原因：当前 APP 需要稳定支持本地完整历史、供应商切换、Responses/Chat Completions
  显式协议切换、自动/手动压缩和断线重试。让后续请求依赖供应商端状态会扩大这些路径的
  交互边界，可能导致切换后上下文不可复用、重试状态不一致或不同供应商行为不一致。
- 保持现状：每轮根据本地持久历史构造完整请求；图片仍按附件记录恢复为上下文输入；压缩
  只改变本地后续请求使用的历史边界，不删除用户可见历史，也不把完整历史交给供应商托管。
- 后续速度优化不得通过删除历史、减少上下文、改变压缩阈值或依赖供应商状态来实现；执行链
  优化以 SSH/远程进程的可验证固定开销为范围，指令缓存必须另有可靠失效证据后才考虑。

### 2026-09-09：稳定性优先的执行速度优化

- 本轮采用的范围：保留本地完整历史和无状态请求；只优化远程命令的固定往返。启用远程
  任务恢复时，`terminal.exec` 现在通过 `RemoteProcessController.startAndPoll` 在一次
  SSH `run` 请求中完成幂等进程启动和首次状态探测，再按 stdout/stderr 字节偏移读取输出。
  短命令因此不再固定经历“启动请求完成后，再单独 poll 一次”的两次命令往返；长命令仍
  返回 `process_id`，后续继续使用原有轮询和断线恢复。
- 保留的不变量：进程 ID 和指纹仍使断线后的启动重试不会重复执行命令；输出仍走分块字节
  读取，不把命令输出混入状态文本；写入、审批、取消、退出码和远端进程记录语义没有改变。
- 明确舍弃的优化：不使用 `previous_response_id`、供应商服务端状态、Responses WebSocket
  增量续接、跨轮无限 SSH 常驻连接或无失效保障的远程指令缓存。这些可能减少传输/握手，
  但会扩大供应商切换、重连、凭据更新或旧指令复用的稳定性边界。
- 验证：`flutter analyze lib/ssh/remote_process.dart lib/agent/agent_tools.dart` 通过；
  `flutter test test/remote_process_test.dart test/agent_loop_test.dart
